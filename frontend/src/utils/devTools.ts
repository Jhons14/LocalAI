...
export class MemoryMonitor {
  ...
  static logMemoryUsage(): void {
    ...
    if (info) {
      logger.debug(
        `Memory usage: ${(info.usedJSHeapSize / 1024 / 1024).toFixed(2)}MB (${info.usedPercent.toFixed(1)}%)`,
        'MEMORY',
        info
      );
    }
...
export function withDebugInfo<P extends object>(
  ...
  const name = debugName || Component.displayName || Component.name || 'Unknown';
  ...
  const DebugWrapper = (props: P) => {
    ...
    React.useEffect(() => {
      ...
      logger.debug(`Component ${name} rendered in ${renderTime.toFixed(2)}ms`, 'RENDER');
    });
